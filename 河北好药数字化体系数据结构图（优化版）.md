# 河北好药数字化体系数据结构图（优化版·码码通专项）

---

## 一、核心设计理念：「码码通」驱动全流程

**核心逻辑：** 以「码码通」微信小程序为唯一移动端入口，以手机号为身份锚点，以二维码为数据载体，实现"一码当先，万码互通"。

**用户体验：** 农户/工人"扫一扫"即可干活，系统通过手机号自动带出待办任务和关联数据，"选一选"即可完成填报，零打字、零培训。
...
#### 1.1 系统用户与手机号绑定表（sys_user_mobile）

核心作用：建立"手机号=身份"的强关联，是所有扫码操作的安全底座。
...
#### 1.2 地块主数据与实体二维码表（gap_land_parcel）

核心作用：地块的物理身份证，扫码即可查看地块档案并进行农事操作。
...
#### 1.3 种苗/农资与批次二维码表（gap_input_batch）

核心作用：投入品的物理身份证，扫码即可记录消耗。
...
#### 2.1 播种记录表（gap_sowing_record）

操作场景：工人扫地块码 → 系统自动带出地块信息 → 扫种苗码 → 选"播种" → 提交。
...
#### 2.2 施肥记录表（gap_fertilization_record）

操作场景：工人扫地块码 → 系统带出地块 → 扫肥料码 → 选"施肥量" → 提交。
...
#### 2.3 打药记录表（gap_pesticide_record）

操作场景：工人扫地块码 → 扫农药码 → 系统自动带出安全间隔期 → 选"稀释倍数" → 提交。
...
#### 2.4 采收记录表（gap_harvest_record）

操作场景：工人扫地块码 → 选"采收" → 输入重量 → 系统生成采收批次码。
...
#### 3.1 收购入库表（pur_receiving）

操作场景：质检员扫采收批次码 → 系统带出产地信息 → 录入质检结果 → 生成内部物料码。
...
#### 3.2 工厂检测表（lab_inspection）

操作场景：化验员扫内部物料码 → 系统带出样品信息 → 选"检验项目" → 录入结果。
...
#### 4.1 生产指令表（prod_order）

操作场景：车间主任扫内部物料码 → 下达生产指令 → 生成成品批号。
...
#### 4.2 工位扫码记录表（prod_station_record）

操作场景：工人扫工单码 → 扫工位码 → 系统自动记录 → 关联IoT数据。
...
#### 6.1 包装赋码表（pkg_trace_code）

操作场景：包装工扫生产工单码 → 系统带出成品信息 → 打印最小包装码 → 建立关联关系。
